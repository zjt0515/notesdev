...
{
   int isNotZero = !!(x);
   int leftOneIsZero = !(x ^ (~x + 1));

   return isNotZero & leftOneIsZero;
}
// 2
/* 先得到一个32位的，全部odd位都是1，其余位0
 * 用他 & x，得到x的奇数位，与32位的数是否相等
...
int conditional(int x, int y, int z)
{
   ...
   return (y & ~x) | (z & x); // x为0，用0xff & z取出z的值，用0x00 & y使|另一边全为0
}
/*
...
unsigned floatScale2(unsigned uf)
{
   ...
   unsigned exp = (uf & 0x7f800000) >> 23; // 阶数
   unsigned frac = uf & (~0xff800000);     // 尾数
   if (exp == 0)
      return frac << 1 | s; // 如果阶数为0，把尾数乘2加上符号位输出
   if (exp == 255)
      return uf;
   exp += 1;
   if (exp == 255)
